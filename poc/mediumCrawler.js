...
function lookupUserQueue() {
    ...
    async.mapLimit(lookups, 50, lookupUModels, function(err, results) {
        //console.log("Lookup Ended" + results.length + " url:" + lookups.length);
...
//=====Update UModels Stats=======================================================
function updateStats(results) {

    var stats = { count: 0, insertCount: 0, existCount: 0, updateCount: 0, notOpCount: 0, score: 0 }
    results.forEach(function(item) {
        let elem = item.uModel;
        stats.count++;
        if (!elem.isExistInDB) stats.insertCount++;
        if (elem.isExistInDB) stats.existCount++;
        if (elem.status) stats.updateCount++;
    });

    stats.notOpCount = stats.count - (stats.insertCount + stats.updateCount);
    stats.score = stats.insertCount - stats.updateCount;
    console.log("Total URLs:%d New URLs:%d Upd URLs:%d NoOp URLs:%d Insert/Update Race:%d", stats.count, stats.insertCount, stats.updateCount, stats.notOpCount, stats.score);


    var sqlTemp = "INSERT INTO CRAWL_STATS (TOTAL_URLS, NEW_URLS, UPD_URLS, NON_URLS, SCORE) VALUES ($totalURLs, $inserURLs,$updURLs,$nonURLs,$score)";
    var sql = sqlTemp.replace("$totalURLs", stats.count).replace("$inserURLs", stats.insertCount).replace("$updURLs", stats.updateCount).replace("$nonURLs", stats.notOpCount).replace("$score", stats.score);
    con.query(sql, function(err, result) {
        if (err) throw err;
        console.log("Stats Persist");
        persistItems();
    });
...
function persistItems() {
    ...
    async.mapLimit(persists, 50, persistUModels, function(err, results) {
        //console.log("Insert Ended" + results.length + " url:" + persists.length);
        if (err != null) console.log(err);
        updateItems();
    });
...
function updateItems() {
    ...
    async.mapLimit(updates, 50, updateUModels, function(err, results) {
        //console.log("Update Ended" + results.length + " url:" + updates.length);
        if (err != null) console.log(err);
        crawlFindRootURL();
    });
}
...
function processUserQueue() {
    ...
    async.mapLimit(urls, 50, loadURLAsync, function(err, results) {
        ...
        if (urls.length <= 100) { //urls.length <= 1
            ...
            if (uModel != undefined) {
                ...
                try {
                    ...
                    for (key in userMap) {
                        ...
                        newUModel.parentURLKey = queueKey;
